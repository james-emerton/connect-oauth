...
var OAuth = exports.OAuth = function OAuth(options) {
    ...
    var plugin = this,
        ...
        authenticate_provider = options.authenticate_provider;
  
    // Both authorize handler and oauth provider must be provided
    if(!options.authenticate_provider)
        throw Error("No Authentication provider provided");
    ...
    this.use(request_token_url, function(req, resp) {
        self.requestToken(req, resp);
    });

    this.use(authorize_url, function(req, resp) {
        if(req.method == 'POST')
            self.authorizeToken(req, resp);
        else
            authenticate_provider(req, resp);
    });

    this.use(access_token_url, function(req, resp) {
        self.accessToken(req, resp);
    });
...
**/
OAuth.prototype.requestToken = function(request, response) { 
...
**/
OAuth.prototype.authorizeToken = function(req, res) {
    var params = req.body,
        oauth_token = req.body.oauth_token,
        verifier = req.body.verifier;
    
    this.oauth_service.authorizeToken(req, oauth_token, function(err, result) {
        if(err) {
...
OAuth.prototype.verifyRequest = function(req, resp, next) {
    this.oauth_service.authorize(req, function(err) {
        if(err) {
            resp.writeHead(err.statusCode, {'Content-Type': 'text/plain'});
            resp.end(err.message);
        }
